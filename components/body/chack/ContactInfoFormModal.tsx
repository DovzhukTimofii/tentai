...
const schema = yup.object().shape({
  fullName: yup.string().required('Полное имя обязательно').min(3, 'Имя должно быть не менее 3 символов'),
  email: yup.string().email('Введите корректный email').required('Email обязателен'),
  phoneNumber: yup.string()
    .matches(/^\+\d{10,15}$/, 'Номер телефона должен начинаться с "+" и содержать от 10 до 15 цифр')
  ...
  return (
    <div className="fixed inset-0 bg-gray-600 bg-opacity-50 flex justify-center items-center z-50 font-inter">
      <div className="bg-white p-8 rounded-lg shadow-xl max-w-md w-full relative">
        <h3 className="text-xl font-semibold mb-6 text-orange-500">Редактировать контактную информацию</h3>
        <button
          onClick={onClose}
        ...
        <form onSubmit={handleSubmit(onSubmit)}>
          ...
          <div className="mb-6">
            ...
            <input
              ...
              placeholder="Например: +66123456789"
            />
